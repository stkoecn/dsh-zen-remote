...
function readSidebarTarget() {
    if (document.querySelector(OFFICIAL_PANEL) !== null)
        return 'official';
    if (document.querySelector(BETTER_ROOT) !== null)
        return 'better';
    return null;
}
/**
...
function toggleSidebarTarget(target) {
    if (target === 'better') {
        document.querySelector(BETTER_TOGGLE)?.click();
        return;
    }
    if (target === 'official') {
        const shown = document.querySelector(`${OFFICIAL_PANEL}[data-sidebar-right-open]`) !== null;
        const control = shown
            ? document.querySelector(OFFICIAL_COLLAPSE)
            : document.querySelector(OFFICIAL_EXPAND);
        control?.click();
    }
}
...
function useSidebarTarget() {
    const [target, setTarget] = (0, react_1.useState)(readSidebarTarget);
    (0, react_1.useEffect)(() => {
        const check = () => setTarget(readSidebarTarget());
